...
uint32_t g_softmaxStaticParamsNum = 0;
...
Udo_ErrorType_t
softmax_createOpFactory (QnnOpPackage_GlobalInfrastructure_t globalInfra,
    ...
    Udo_Param_t *staticParams, Udo_OpFactory_t *opFactory)
{
    ...
    if(strcmp(operationType, g_softmaxOpType) == 0) {
        softmaxOpFactory_t* thisFactory = (softmaxOpFactory_t *)(*(globalInfra->dspGlobalInfra->hexNNv2Infra.udoMalloc))(sizeof(softmaxOpFactory_t));
...
Udo_ErrorType_t
softmax_releaseOpFactory(QnnOpPackage_GlobalInfrastructure_t globalInfra,
...
Udo_ErrorType_t
softmax_validateOperation (Udo_String_t operationType, uint32_t numOfStaticParams,
...
Udo_ErrorType_t
softmax_executeOp (QnnOpPackage_GlobalInfrastructure_t globalInfra,
    ...
    const char* opType = ((softmaxOpFactory_t*)(m_Operation->opFactory))->opType;
    if(opType == NULL) {
        return UDO_INVALID_ARGUMENT;
    }
    if(strcmp(opType, g_softmaxOpType) == 0) {
        ...
        SoftmaxOpInfo workerThreadIn = {inputTensorData, inputLen,
                                        ...
                                        outputTensorData};
        (*(globalInfra->dspGlobalInfra->hexNNv2Infra.udoRunWorkerThreads))(m_Operation->opInfra, 1, worker_thread_SoftmaxQuant, &workerThreadIn);
...
Udo_ErrorType_t softmax_queryOperation (
    Udo_String_t operationType, uint32_t numOfStaticParams,
...
Udo_ErrorType_t free_softmax(QnnOpPackage_GlobalInfrastructure_t globalInfra, UdoDspShared* opInfo) {
    if (opInfo == NULL) {
        return UDO_NO_ERROR;
    }
