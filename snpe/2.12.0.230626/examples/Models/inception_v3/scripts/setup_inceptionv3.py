...
def prepare_data_images(snpe_root, model_dir, tensorflow_dir):
    ...
    os.makedirs(data_dir + '/cropped', exist_ok=True)
...
def convert_to_dlc(pb_filename, model_dir, tensorflow_dir, runtime, udo, htp_soc):
    ...
    if udo:
        cmd.append('--udo')
        if htp_soc=="sm8550":
            cmd.append(os.path.join(UDO.SNPE_UDO_PATH, 'config', UDO.INCEPTION_V3_UDO_PLUGIN_HTP_V73))
        elif htp_soc=="sm8450" or htp_soc=="sm8350":
...
def setup_assets(inception_v3_data_dir, download, runtime, udo, htp_soc):
    ...
    model_dir = os.path.join(snpe_root, 'examples', 'Models', 'inception_v3')
    ...
    if udo:
        ...
        udo_quantized_package_path = os.path.join(model_dir, "quant_package")
        if not os.path.isdir(udo_quantized_package_path):
            os.mkdir(udo_quantized_package_path)
